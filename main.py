...
def write_mihomo_files(ordered) -> tuple[int, int]:
    ...
    # RAW GitHub only. Cloudflare Worker / workers.dev support removed.
    # Provider is always generated from the repository RAW URL.
    provider_url = ""
    if repository:
        provider_url = (
...
async def main():
    ...
    # Every run rebuilds the candidate pool from:
    # 1) freshly collected source keys;
    # 2) previously accepted keys from state.
    # The combined pool is filtered again from zero.
    # Any key that fails the current verification run is removed immediately.
    ...
    for key, result in zip(keys, results):
        ...
        if result.ok:
            ...
            # No retry counters: a failed probe means immediate removal.
            del current[key]
            deleted += 1
    ...
    status = {
        ...
        "admission_rule": "A key is published only if it passes all filters and probes in the current run. Failed keys are removed immediately.",
