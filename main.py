        ...
        try:
            proxies.append(
                mihomo_proxy_from_uri(
                    item["uri"],
                    unquote(urlsplit(item["uri"]).fragment) if urlsplit(item["uri"]).fragment else f"node-{index}",
                )
            )
        except Exception:
            skipped += 1
...
async def main():
    ...
    status = {
        ...
        "output_validation": {"mihomo_nodes": mihomo_nodes, "conversion_failed": mihomo_skipped},
