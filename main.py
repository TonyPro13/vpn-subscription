...
def candidate_dedup_key(uri: str) -> str:
    """
    Hiddify-compatibility candidate key.

    Different real servers/configurations remain separate. We collapse only
    client-fingerprint variants (fp/fingerprint) of the SAME endpoint/config.
    The selected URI itself is never rewritten.

    Preserved:
    protocol, endpoint host/domain, port, credentials, transport, path, mode,
    SNI, REALITY public key/short id and every other operational parameter.

    Ignored:
    display name/fragment and client TLS fingerprint only.
    """
    uri = uri.strip()
    try:
        scheme = uri.split("://", 1)[0].lower()
        ...
        if scheme == "vmess":
            ...
            if data.get("port") is not None:
                data["port"] = str(data["port"])
            return "vmess-family:" + json.dumps(
                data,
                ensure_ascii=False,
                sort_keys=True,
                separators=(",", ":"),
        ...
        if scheme == "ss":
            return "ss-family:" + canonical(uri)
        ...
        query = [
            ...
            if key.lower() not in {"fp", "fingerprint"}
        ]
        payload = {
            "scheme": canonical_scheme,
            "userinfo": uri_userinfo(uri),
        ...
        return "raw-family:" + uri.split("#", 1)[0].strip()
...
def collect_sources():
    ...
    for url in SOURCES:
        ...
        for line in source_text.splitlines():
            ...
            source_stats[url] += 1
            # First source occurrence wins; original URI is preserved unchanged.
            unique[key] = uri
...
async def main():
    ...
    # state.json is only candidate memory. Every surviving node will still pass
    # all mandatory checks. Current source entries are placed first so that a
    # current URI wins over an older state fp-variant of the same configuration.
    candidates = list(source_nodes.values())
    ...
    # Reapply the same compatibility dedup after merging state, without changing
    # the selected URI itself.
    merged = {}
    for uri in candidates:
        merged.setdefault(candidate_dedup_key(uri), uri)
    ...
    # No internal ranking. Mihomo / Hiddify choose the active node.
    ...
    status = {
        ...
        "candidate_dedup_policy": "same endpoint/config collapsed only across client fp/fingerprint variants; first current-source URI wins",
