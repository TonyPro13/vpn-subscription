...
def identity_key(uri: str):
    """
    Conservative identity key:
    removes obvious duplicate aliases without merging unrelated servers.
    """
    try:
        scheme = uri.split("://", 1)[0].lower()
        if scheme == "vless":
            p = urlsplit(uri)
            return ("vless", p.username, p.port)
        if scheme == "trojan":
            p = urlsplit(uri)
            return ("trojan", p.username, p.port)
        if scheme == "vmess":
            raw = uri.split("://", 1)[1].split("#", 1)[0]
            data = json.loads(b64decode(raw).decode("utf-8", errors="replace"))
            return ("vmess", data.get("id"), data.get("port"))
        if scheme in {"ss", "hy2", "hysteria2"}:
            p = urlsplit(uri)
...
def geo_country_label(host: str):
    """
    Optional country resolver for output labels.
    Filtering logic remains independent from this function.
    If GeoIP database is unavailable, country stays unknown.
    """
    try:
        import geoip2.database
        if not GEOIP_COUNTRY_DB.exists():
            return None
        ip = ipaddress.ip_address(host)
        with geoip2.database.Reader(str(GEOIP_COUNTRY_DB)) as reader:
            country = reader.country(str(ip)).country
            if country and country.names:
                return country.names.get("en") or country.iso_code
    except Exception:
        return None
    return None
...
def detect_country_label(host: str):
    """
    Placeholder for future country mapping.
    Current geo filter only knows RU/non-RU.
    """
    return None
...
def collect_sources():
    ...
    for url in SOURCES:
        ...
        for line in text.splitlines():
            ...
            k = canonical(s)
            ident = identity_key(s)
            duplicate_key = (k, ident)

            if k in unique or any(identity_key(v) == ident and ident for v in unique.values()):
                duplicates += 1
                continue
...
async def run_probe(uri: str, probe_stats: dict):
    """
    Creates a temporary VPN tunnel and runs the mandatory quality cascade.
    Supports Xray based protocols and sing-box Hysteria2.
    """
    engine = None
    process = None
    port = free_port()
...
def mihomo_transport_options(proxy: dict, q: dict):
    """
    Preserve transport parameters when converting Xray style URI to Mihomo.
    """
    network = q1(q, "type", "tcp")
...
def mihomo_proxy_from_uri(uri: str, name: str):
    """
    Convert supported VPN URI into a real Mihomo proxy object.
    """
    p = urlsplit(uri)
    q = parse_qs(p.query, keep_blank_values=True)
    scheme = p.scheme.lower()
    ...
    if scheme == "vless":
        ...
        if security in {"tls", "reality"}:
            proxy["tls"] = True
            proxy["servername"] = q1(q, "sni", "")
            if q1(q, "fp", ""):
                proxy["client-fingerprint"] = q1(q, "fp", "")
        if security == "reality":
            proxy["reality-opts"] = {
                "public-key": q1(q, "pbk", ""),
                "short-id": q1(q, "sid", ""),
        ...
        if q1(q, "flow", ""):
            proxy["flow"] = q1(q, "flow", "")
        return mihomo_transport_options(proxy, q)
    ...
    if scheme == "vmess":
        ...
        proxy = {
            ...
            "network": o.get("net", "tcp"),
            "tls": str(o.get("tls", "")).lower() == "tls",
            "servername": o.get("sni", ""),
            "udp": True,
        }
        ...
        return mihomo_transport_options(proxy, fake_q)
    ...
    if scheme in {"hy2", "hysteria2"}:
        proxy = {
            ...
            "skip-cert-verify": q1(q, "insecure", "0").lower() in {"1", "true"},
            "udp": True,
        }
        obfs = q1(q, "obfs", "")
        if obfs:
        ...
        if q1(q, "alpn", ""):
            proxy["alpn"] = [x for x in q1(q, "alpn", "").split(",") if x]
        if q1(q, "upmbps", ""):
            proxy["up"] = q1(q, "upmbps", "")
        if q1(q, "downmbps", ""):
            proxy["down"] = q1(q, "downmbps", "")
        return proxy
...
def write_mihomo_files(nodes):
    """
    Creates real Mihomo Meta compatible configuration files.
    Nodes are not ranked. AUTO selection is delegated to the client.
    """
    ...
    for index, item in enumerate(nodes, start=1):
        node_name = (
            (f"{item.get('country')} | " if item.get("country") else "")
            + (
                unquote(urlsplit(item["uri"]).fragment)
                if urlsplit(item["uri"]).fragment
                else f"{urlsplit(item['uri']).scheme}-{urlsplit(item['uri']).hostname or index}"
            )
        )
        try:
            proxies.append(
                mihomo_proxy_from_uri(
                    item["uri"],
                    node_name,
                )
            )
...
def yaml_dump(data):
    """
    Minimal YAML serializer without external dependencies.
    """
    def scalar(value):
        if value is None:
            return "null"
...
async def safe_probe(uri: str, probe_stats: dict):
    ...
    Starts the temporary Xray/sing-box tunnel and runs the mandatory
    service cascade. Any exception means the node fails immediately.
    """
...
async def main():
    ...
    for uri in candidates.values():
        uri = clean_insecure_params(uri)
        key = canonical(uri)
        ident = identity_key(uri)

        if key in merged:
            continue

        if ident and ident in merged_identity:
            continue

        merged[key] = uri
        if ident:
            merged_identity[ident] = key
    ...
    current = {
        key: {
            ...
            "country_source": "geoip" if detect_country_label_from_host(extract_server_host(uri)) else "unknown",
    ...
    status = {
        ...
        "run_duration_seconds": round(time.monotonic() - run_started, 2),
